...
algorithms = [algos.Algo0,algos.Algo1,algos.AlgoPixelCenter,algos.AlgoPixelCorner,algos.AlgoRasterScan]
# algos = [contourAlgo.AlgoPixelCenter]

plt.rcParams['xtick.bottom'] = plt.rcParams['xtick.labelbottom'] = False
plt.rcParams['xtick.top'] = plt.rcParams['xtick.labeltop'] = True
#%% Creating Image
image = construct.imageV()
...
twist = 0
G = nx.Graph()
G = networks.graph_twisted_torus(G,x,y,twist)
#print(G.edges)
#print(G.nodes)
# for me in G.nodes():
...
nx.set_node_attributes(G,1,"pixel")         # didnt understand this
for node in G.nodes : 
    #print(node)
    for row in range(x):
        for col in range(y):
...
for algo in algorithms:
    ...
    records = open ('Hops_stacklen.csv', 'a', newline='')
    contourStack = []
    contourStack = algo(G,x,y,contourStack)
